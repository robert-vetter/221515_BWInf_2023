...
//Parameter
int n = 1000;//Anzahl an maximal erscheinenden Polygonen

float time = 2; //Jede (time/60)s wird ein neuer Keim geschaffen

float vOmax = 0.05; //Maximale Wachstumsgeschwindigkeit in Richtung der Orientierung des Kristalls
float vOmin = 0.04; //Minimale Wachstumsgeschwindigkeit in Richtung der Orientierung des Kristalls
float vSmax = 0.04; //Maximale Wachstumsgeschwindigkeit in anderen Wachstumsrichtungen
float vSmin = 0.03; //Minimale Wachstumsgeschwindigkeit in anderen Wachstumsrichtungen

boolean manualMode = false; //mithilfe des manuellen Modus können eigene Polygone gezeichnet werden und so die genaue Position der Keime festgelegt werden
...
//Liste von Objekten initialisieren(Polygonen)
ArrayList<Polygon> polygons;

//Farbe des 1. Polygons
color cl;
...
//Ortskoordinaten für die Polygone
float x1;
float y1;
float x2;
float x3;
...
void setup(){
  ...
  if(manualMode){
    println("Sie befinden sich im manuellen Modus und können nun selber unter Betätigung der linken Maustaste Polygone zeichnen");
  } else {
    println("Sie befinden sich im Simulationsmodus");
    println("Es werden insgesamt " + n + " Polygone gezeichnet");
  ...
  //zu dieser ArrayList ein neues Polygon mit den passenden Parametern hinzufügen
  polygons.add(new Polygon(x1, y1, x2, y2, x3, y3, x4, y4, cl, vLeft, vUp, vRight, vDown));
}

//Hauptmethode, in welcher Polygone gezeichnet werden und auf Kollisionen überprüft wird
void draw(){
  
   Polygon newP = newPolygon();
  
    //wenn nicht mehr als n Elemente enthalten sind und ein neues Polygon newP gezeichnet wurde, dann soll dieses der ArrayList hinbzugefügt werden
    //draw() wird 60 Mal pro Sekunde aufgerufen
    if (newP!=null && polygons.size() < n && frameCount % time == 0){
      polygons.add(newP);
     }
    
    //for-Schleife, die für jedes Polygon ausgeführt wird
    for (Polygon p : polygons){
      //solange sich mindestens eine Ecke des Polygons noch ausbreitet:
      if (p.growingLeft || p.growingRight || p.growingUp || p.growingDown){
        ...
        for (Polygon other : polygons){
          //wenn "other" ein anderes Polygon als "p" beschreibt
          if (p != other){
...
Polygon newPolygon(){
   ...
   //Zeichnen der Polygone im manuellen Modus
   ...
   if (selection == 1){
     if (x1 < 250  && y1 > 200 ){
           ...
           vLeft = random(vOmin, vOmax);
           vUp = random(vSmin, vSmax);
           vRight = random(vSmin, vSmax);
           vDown = random(vSmin, vSmax);
      }
      if (x1 < 150 && (y1 > 75 && y1 < 250)){
          cl = int(random(125, 175));
          vRight = random(vOmin, vOmax);
          vDown = random(vSmin, vSmax);
          vLeft = random(vSmin, vSmax);
          vUp = random(vSmin, vSmax);
      }
      if (x1 < 170  && y1 < 170 ){
            cl = int(random(175, 200));
            vUp = random(vOmin, vOmax);
            vDown = random(vSmin,vSmax);
            vLeft = random(vSmin, vSmax);
            vRight = random(vSmin, vSmax);
      }
   ...
   //boolean, die ausdrückt, ob das Polygon gezeichnet werden darf oder nicht
   boolean valid = true;
   ...
   for (Polygon p : polygons){
     //4 Vektoren werden erstellt
      ...
      boolean hit = PolygonPointCollision(vektoren, x1,y1);
     //wenn das neue Polygon in einem schon bestehenden Polygon neu erscheinen will, wird dies nicht zugelassen
     if (hit){
       valid = false;
       break;
     }
   ...
   //wenn das neue Polygon außerhalb erscheint, wird das Zeichnen genehmigt, sonst nicht
   if (valid){
       return new Polygon(x1, y1, x2, y2, x3, y3, x4, y4, cl, vLeft, vUp, vRight, vDown);
   } else {
     return null;
   }
...
//hier wird überprüft, ob ein Polygon in einem anderen erscheint
boolean PolygonPointCollision(PVector[] vektoren, float x1, float y1) {
  int next = 0;
